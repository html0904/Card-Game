 ...
 */
public class RiposteScenario implements TestScenario{
    private Tournament tournament;
    private List<Player> players;
    private int clientID;

    public RiposteScenario(int id) {
        clientID = id;
        ...
        p1.addCardToHand(new Card(0, 0, Properties.ACTION.RIPOSTE, Properties.COLOR.NONE));
    ...
    public GameState getGameState(PlayerAction action) {
        ...
        if (action.getActionTaken() == Properties.GAME_ACTION.SELECT_HAND_CARD) {
            ...
            return gs;
        } else if (action.getActionTaken() == Properties.GAME_ACTION.SELECT_OPPONENT_DISPLAY) {
            GameState gs = tournament.performAction(action);
            gs.setTargetPlayerID(clientID);
            return gs;
        }
        return new GameState(players, Properties.GAME_ACTION.END_OF_TEST, clientID, Properties.COLOR.PURPLE,
                tournament.getLastCardPlayed());
    }
}
