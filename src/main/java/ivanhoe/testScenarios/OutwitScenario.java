 ...
 */
public class OutwitScenario implements TestScenario {

    private Tournament tournament;
    private List<Player> players;
    private int clientID;

    public OutwitScenario(int id) {
        clientID = id;
        ...
        p1.addCardToDisplay(new Card(1, 0, Properties.ACTION.SHIELD, Properties.COLOR.NONE));
        ...
        p2.addCardToDisplay(new Card(2, 2, Properties.ACTION.NONE, Properties.COLOR.YELLOW));
    ...
    public GameState getGameState(PlayerAction action) {
        ...
        if (action.getActionTaken() == Properties.GAME_ACTION.SELECT_HAND_CARD
                || action.getActionTaken() == Properties.GAME_ACTION.SELECT_DISPLAY_CARD
                || action.getActionTaken() == Properties.GAME_ACTION.SELECT_OPPONENT_DISPLAY_CARD) {
            GameState gs = tournament.performAction(action);
            gs.setTargetPlayerID(clientID);
            return gs;
        }
        return new GameState(players, Properties.GAME_ACTION.END_OF_TEST, clientID, Properties.COLOR.YELLOW
                , new Card(12, 0, Properties.ACTION.OUTWIT, Properties.COLOR.NONE));
